...
genotypeFactor.card(1) = (numAlleles + 1)*numAlleles/2; 
genotypeFactor.card(2) = genotypeFactor.card(1);
genotypeFactor.card(3) = genotypeFactor.card(1);

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.

for index = 1:length(genotypeFactor.val)
  genotypes = IndexToAssignment(index, genotypeFactor.card);
  
  % I extract all the possible allele assignment states corresponding to all
  % the combination of mother's, father's and child's genotypes given 
  % genotypeFactor.card(1) possible genotypes, for each genotype I consider 
  % the corresponding allele couple and compute the probability for the 
  % corresponding allele couple of the child to be generated by mother's 
  % and father's allele couple, each parent contribute for one of the child's alleles.
  
  child_alleles = genotypesToAlleles(genotypes(1),:);
  mother_alleles = genotypesToAlleles(genotypes(2),:);
  father_alleles = genotypesToAlleles(genotypes(3),:);
  
  if child_alleles(1) ~= child_alleles(2)
    % case of mother contributing to allele one and father contributing to allele two
    genotypeFactor.val(index) = (sum([mother_alleles(1), mother_alleles(2)] == child_alleles(1))/2)*(sum([father_alleles(1), father_alleles(2)] == child_alleles(2))/2);
    % viceversa
    genotypeFactor.val(index) += (sum([mother_alleles(1), mother_alleles(2)] == child_alleles(2))/2)*(sum([father_alleles(1), father_alleles(2)] == child_alleles(1))/2);
  elseif child_alleles(1) == child_alleles(2)
    genotypeFactor.val(index) = (sum([mother_alleles(1), mother_alleles(2)] == child_alleles(1))/2)*(sum([father_alleles(1), father_alleles(2)] == child_alleles(2))/2);  
  end;
